class Solution {
    private void buildAdj(List<List<Integer>> adj, int edges[][]){
        for(int edge[] : edges){
    ...
    private void getDist(int p[], List<List<Integer>> adj, int node, int dist, boolean vis[]){
        int d = p[1];
        vis[node] = true;
        ...
        for(int neighbor : adj.get(node)){
            if(!vis[neighbor]){
                getDist(p, adj, neighbor, dist + 1, vis);
            }
        }
    }
    ...
    public int minimumDiameterAfterMerge(int[][] edges1, int[][] edges2) {
       ...
       getDist(p1, adj1, 0, 0, vis);
       int p2[] = new int[]{-1, Integer.MIN_VALUE};
       Arrays.fill(vis, false);
       getDist(p2, adj1, p1[0], 0, vis);
       int d1 = p2[1];

       p1 = new int[]{-1, Integer.MIN_VALUE};
       vis = new boolean[m];
       getDist(p1, adj2, 0, 0, vis);
       p2 = new int[]{-1, Integer.MIN_VALUE};
       Arrays.fill(vis, false);
       getDist(p2, adj2, p1[0], 0, vis);
       int d2 = p2[1];
